...
import { Autocomplete, Box, Fab, Grid, Paper, Stack, TextField, Typography } from '@mui/material';
...
import { useImage, useImageModels } from '../hooks/useAPI';
import './Image.css';
...
export default function Image({ selectedChat, onChatSave, setSavedPromptOpen }) {
  ...
  const onMessagesSubmit = async (prompt) => {
    ...
    try {
      const { data } = await axios.post(`${VITE_CHAT_API_URL}/openai/image/create`, { prompt, config: { model } });
